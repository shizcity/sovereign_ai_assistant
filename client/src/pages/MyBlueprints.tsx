                ...
                <div key={bp.id} className="p-4 rounded-xl border border-white/10 bg-white/3 hover:bg-white/5 transition-colors">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      {bp.isPublic && (
                        <>
                          <button onClick={() => handleCopyLink(bp.shareToken, bp.id)} className="p-1.5 rounded-lg text-white/30 hover:text-white/70 hover:bg-white/5 transition-colors" title="Copy share link">
                      ...
                      <button onClick={() => toggleMutation.mutate({ id: bp.id, isPublic: !bp.isPublic })} className="p-1.5 rounded-lg text-white/30 hover:text-white/70 hover:bg-white/5 transition-colors" title={bp.isPublic ? "Make private" : "Make public"}>
                        {bp.isPublic ? <Lock className="w-4 h-4" /> : <Globe className="w-4 h-4" />}
                      </button>
                      <button onClick={() => { if (confirm("Delete this blueprint?")) deleteMutation.mutate({ id: bp.id }); }} className="p-1.5 rounded-lg text-white/30 hover:text-red-400 hover:bg-red-500/10 transition-colors" title="Delete blueprint">
